						...
						<li key={p.id}>
							<Link href={"/producto/" + p.id} aria-label={"Ver producto " + p.name} className="group block overflow-hidden">
								...
								<div className="relative pt-3 bg-base-100">
									<h3 className="text-xs text-gray-700 dark:text-slate-400 group-hover:underline group-hover:underline-offset-4">{p.name}</h3>
									<p className="mt-2"> <span className="tracking-wider text-gray-900 dark:text-white">{"$" + p.price / 100}</span> </p>
								</div>
							</Link>
						</li>)
				}
