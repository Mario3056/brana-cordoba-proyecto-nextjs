        ...
        <>
            <section className="mt-12 max-w-screen-lg mx-auto px-4 md:px-8">
                <ul className="mt-12 space-y-6">
                    { products.map((product, idx) => <DeletedTableRow product={product} key={idx} />) }
                </ul>
            </section>
        </>
    )
...
function DeletedTableRow({product}: { p: Product} ) {
	...
	return <li className="p-5 bg-white rounded-md shadow-sm">
			<div>
				<div className="justify-between sm:flex">
					...
					<div className="flex-1">
						<p className="text-xl font-medium text-indigo-600"> {product.name} </p>
